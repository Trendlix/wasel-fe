...
const FAQ = ({
    ...
    const count = faqContent?.items?.length || 7;
                    ...
                    <Accordion type="single" collapsible>
                        {Array.from({ length: count }, (_, i) => (
                            <AccordionItem className="border-b last:border-b-0 border-main-grayHint" key={i} value={`item-${i}`}>
                                <AccordionTrigger>{faqContent?.items?.[i]?.question || tItems(`${i}.q`)}</AccordionTrigger>
                                <AccordionContent>{faqContent?.items?.[i]?.answer || tItems(`${i}.a`)}</AccordionContent>
                            </AccordionItem>
                        ))}
                    </Accordion>
                </div>

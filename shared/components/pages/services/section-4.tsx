...
import { serviceSection4Cards, type IServiceSection4Card } from "@/shared/constants/services";
...
const Section4 = () => {
    ...
    useGSAP(() => {
        // Heading: fade up + widthX bar (like section-3)
    ...
    return (
        <section ref={scopeRef}>
            <div className="container px-4 sm:px-6 lg:px-8 py-12 sm:py-16 md:py-20 xl:py-28">
                <Heading ref={headingRef} t={t} dir={dir} setHeadingXWidthRef={setHeadingXWidthRef} />
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 lg:gap-8 mt-6 sm:mt-8 md:mt-10 lg:mt-12">
                    {serviceSection4Cards.map((card, index) => {
...
const Card = ({ card, content, reverse }: { card: IServiceSection4Card; content: { tag: string; title: string; points: string[] }; reverse: boolean }) => {
